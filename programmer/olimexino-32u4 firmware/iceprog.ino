...
void begin_sram()
{
  ...
  delay(5);
  digitalWrite(CS, HIGH);
  SPI.transfer(0);  // 8 clk with /CS high
  digitalWrite(CS, LOW);
  startframe(READY);
  sendframe();
}
...
{
  for (int x = 0; x < 15; x++)
    SPI.transfer(0);  // Min 49 clk
  // CDONE should now be HIGH
}
